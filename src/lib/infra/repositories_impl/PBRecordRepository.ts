...
export class PBRecordsRepository implements IRecordRepository {
	public async getRecordsBySchoolIdAndMonth(token: string, schoolId: string, month: number): Promise<Record[]> {
		try {
			...
			const response = await axios.get(`${API_BASE_URL}/collections/measurements_records/records?expand=student_id,student_id.class_id,student_id.class_id.school_id&filter=(student_id.class_id.school_id='${schoolId}'%26%26date>='${startOfMonth}'%26%26date<='${endOfMonth}')&perPage=500`, {
				headers: {
